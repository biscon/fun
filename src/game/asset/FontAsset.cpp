...
AssetStatus FontAsset::getStatus() {
    return status;
}

void FontAsset::setStatus(AssetStatus as) {
    status = as;
}
...
bool FontAsset::load(IGame& game) {
...
bool FontAsset::prepare(IGame &game) {
    for(uint8_t chr = 0; chr <= 255; ++chr)
    {
        ...
        auto handle = game.getRenderer()->getTextureManager().createTexture(pb);
